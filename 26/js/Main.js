...
Dog.Main = (function() {
	...
	function animateElements() {
		for (var i = 0; i < slices.length; i++) {
			...
			var distanceRoundCirc = Dog.Utils.map(elementCirc, 0, maxCirc, 0, (Math.PI * 8));
			...
			slice.rotation.z += 0.05;

			var newSlicePos = slice.positionClone.clone().multiplyScalar(Dog.Utils.map(Math.sin(distanceRoundCirc), -1, 1, 0.75, 1.25));
			slice.position.x = newSlicePos.x;
			slice.position.z = newSlicePos.z;


			for (var j = 0; j < slice.children.length; j++) {
				var element = slice.children[j];
				var newScale = element.scaleClone.x * Dog.Utils.map(Math.sin(distanceRoundCirc), -1, 1, 0.5, 1.5);
				element.scale.x = element.scale.y = element.scale.z = newScale;

				var newPos = element.positionClone.clone().multiplyScalar(Dog.Utils.map(Math.sin(distanceRoundCirc), -1, 1, 0.5, 1.5));
	...
	function addElements() {
		...
		for (var i = 0; i < 360; i += 12) {
			...
			slice.positionClone = slice.position.clone();
			...
			for (var j = 0; j < howManyPerSlice; j++) {
				...
				element.scale.x = element.scale.y = element.scale.z = Dog.Utils.map(distance, (donutRadius - innerRadiusMax), (donutRadius + innerRadiusMax), 0.8, 1.2);
