...
Dog.Main = (function() {
	...
	function addLighting() {
		...
		spotLight1.position.set(Dog.Utils.randomRange(300, 500), Dog.Utils.randomRange(800, 1000), Dog.Utils.randomRange(300, 500));
		...
		spotLight1.shadow.camera.far = 1600;
	...
	function addElements() {
		...
		var geometry = new THREE.CylinderGeometry(cylinderRadius, cylinderRadius, cylinderHeight, 48, 60, false);
	...
	function animateHeroFaces() {
		...
		for (var i = 0; i < hero.geometry.vertices.length; i++) {
			...
			if (vertCloned2DLength > 0) {
				...
				newVect.setLength(vertCloned2DLength + ((Math.sin(Dog.Utils.map(animOffset + vertCloned.y, 0, 360, 0, Math.PI * 12)) * maxUndulation)));
				vert.x = newVect.x;
				vert.z = newVect.y;
			}
		}
	...
	function animateCamera() {
		...
		var length = new THREE.Vector2(camera.position.x, camera.position.z).length();

		camera.position.x = Math.cos(Dog.Utils.degToRad(animOffset)) * length;
		camera.position.z = Math.sin(Dog.Utils.degToRad(animOffset)) * length;
