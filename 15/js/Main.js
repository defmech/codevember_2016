...
Dog.Main = (function() {
	...
	function addLighting() {
		...
		spotLight1.shadow.camera.far = 600;
	...
	function addElements() {
		...
		for (var i = 0; i < howMany; i++) {
			...
			var phi = Dog.Utils.randomRange(0, Math.PI * 2);
			var costheta = Dog.Utils.randomRange(-1, 1);
			var u = Dog.Utils.randomRange(0, 1);

			var theta = Math.acos(costheta);
			var r = donutRadius * Math.cbrt(u);

			element.position.x = r * Math.sin(theta) * Math.cos(phi);
			element.position.y = Y_MIN + (r * Math.sin(theta) * Math.sin(phi));
			element.position.z = r * Math.cos(theta);

			element.rotation.y = Math.PI - angleInsideDonut;
